...
from .config import BeltalowdaConfig
from .models import (
...
class BeltalowdaOrchestrator(BaseOrchestrator):
    """
    Hierarchical orchestrator using Beltalowda pattern
    ...
    def __init__(
        self,
        config: Optional[BeltalowdaConfig] = None,
        provider: Any = None,
        model: Optional[str] = None
    ):
        """
        Initialize Beltalowda orchestrator
        ...
        elif not isinstance(config, BeltalowdaConfig):
            # Convert generic config to Beltalowda config
            config = BeltalowdaConfig(**config.to_dict())
