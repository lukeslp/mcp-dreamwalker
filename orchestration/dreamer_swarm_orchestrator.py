...
This wrapper inherits from BaseOrchestrator while maintaining the
original Swarm Search functionality.
...
from .config import DreamerSwarmConfig
from .models import AgentResult, AgentType, SubTask, TaskStatus

logger = logging.getLogger(__name__)


class DreamerSwarmOrchestrator(BaseOrchestrator):
    """
    Multi-agent search orchestrator with specialized agent types (Dreamer agent)
    ...
    AGENT_TYPES = {
        ...
        "image": {
            "icon": "fa-image",
            "color": "#e74c3c",
            "description": "Image search and analysis",
        },
    ...
    def __init__(
        self,
        config: Optional[DreamerSwarmConfig] = None,
        provider: Any = None,
        model: Optional[str] = None,
    ):
        """
        Initialize Dreamer Swarm orchestrator

        Args:
            config: Dreamer Swarm configuration
            provider: LLM provider instance
            model: Model name (optional, uses config.primary_model if not specified)
        """
        # Use Swarm config or create default
        if config is None:
            config = DreamerSwarmConfig()
        elif not isinstance(config, DreamerSwarmConfig):
            # Convert generic config to Swarm config
            config = DreamerSwarmConfig(**config.to_dict())
...
- No conversational padding"""
        ...
        try:
            ...
            return AgentResult(
                ...
                cost=cost,
                metadata={"swarm_type": agent_type, "specialization": subtask.specialization},
            )
...
- Provide a coherent narrative"""
        ...
        for result in successful_results:
            ...
            agent_findings.append(f"## {agent_type.title()} Search Results\n{result.content}")
...
Provide a well-structured, comprehensive summary that integrates all the information gathered by the different agents."""
    ...
    def _determine_agent_type(self, task_description: str) -> str:
        ...
        if (
            any(word in task_lower for word in ["image", "photo", "picture", "visual"])
            and "image" in allowed_types
        ):
            return "image"
        elif (
            any(word in task_lower for word in ["video", "youtube", "clip", "footage"])
            and "video" in allowed_types
        ):
            return "video"
        elif (
            any(word in task_lower for word in ["news", "current", "latest", "recent"])
            and "news" in allowed_types
        ):
            return "news"
        elif (
            any(
                word in task_lower for word in ["academic", "research", "paper", "study", "journal"]
            )
            and "academic" in allowed_types
        ):
            return "academic"
        elif (
            any(word in task_lower for word in ["social", "twitter", "reddit", "forum"])
            and "social" in allowed_types
        ):
            return "social"
        elif (
            any(word in task_lower for word in ["product", "review", "buy", "purchase", "price"])
            and "product" in allowed_types
        ):
            return "product"
        elif (
            any(
                word in task_lower
                for word in ["code", "technical", "documentation", "api", "programming"]
            )
            and "technical" in allowed_types
        ):
            return "technical"
        elif (
            any(word in task_lower for word in ["text", "article", "blog", "content"])
            and "text" in allowed_types
        ):
            return "text"
        else:
            return "general"
